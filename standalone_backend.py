...
async def get_signal_insights(symbol: str):
    ...
    return {
        ...
        "bullish_signals": len([s for s in signals if s.type == "BUY"]),
        "bearish_signals": len([s for s in signals if s.type == "SELL"]),
        "average_confidence": np.mean([s.confidence for s in signals]) if signals else 0,
        "recommendation": "BUY" if len([s for s in signals if s.type == "BUY"]) > len([s for s in signals if s.type == "SELL"]) else "HOLD",
        "timestamp": datetime.now(timezone.utc).isoformat()
    }
