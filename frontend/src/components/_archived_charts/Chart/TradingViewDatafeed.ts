...
export class TradingViewDatafeed implements IExternalDatafeed {
    ...
    constructor() {
        logger.info('TradingView Datafeed initialized with real market data');
    }
    ...
    onReady(callback: (configurationData: any) => void): void {
        logger.info('[onReady]: Method called');
    ...
    searchSymbols(
        ...
        logger.info('[searchSymbols]: Method called', { userInput, exchange, symbolType });
        ...
        marketDataService.searchSymbols(userInput, symbolType, exchange)
            .then(symbols => {
                ...
                logger.info('[searchSymbols]: Returning', searchResults.length, 'results');
                onResultReadyCallback(searchResults);
            })
            .catch(error => {
                logger.error('[searchSymbols]: Error', error);
    ...
    resolveSymbol(
        ...
        logger.info('[resolveSymbol]: Method called', symbolName);
        ...
        marketDataService.getSymbolInfo(cleanSymbol)
            .then(symbolInfo => {
                ...
                logger.info('[resolveSymbol]: Symbol resolved', tradingViewSymbolInfo);
                onSymbolResolvedCallback(tradingViewSymbolInfo);
            })
            .catch(error => {
                logger.error('[resolveSymbol]: Error resolving symbol', error);
    ...
    getBars(
        ...
        logger.info('[getBars]: Method called', {
        ...
        marketDataService.getHistoricalData(symbolInfo.name, resolution, from, to)
            .then((bars: MarketBar[]) => {
                logger.info(`[getBars]: Retrieved ${bars.length} bars for ${symbolInfo.name}`);
            ...
            .catch(error => {
                logger.error('[getBars]: Error fetching historical data', error);
    ...
    subscribeBars(
        ...
        logger.info('[subscribeBars]: Method called', {
            symbol: symbolInfo.name,
            resolution,
            subscriberUID,
        });
        ...
        const subscriptionId = marketDataService.subscribeToRealTimeData(
            symbolInfo.name,
            (quote: MarketQuote) => {
                logger.info('[subscribeBars]: Real-time quote received', quote);
        ...
        logger.info(`[subscribeBars]: Subscribed to ${symbolInfo.name} with ID ${subscriptionId}`);
    }
    ...
    unsubscribeBars(subscriberUID: string): void {
        logger.info('[unsubscribeBars]: Method called', subscriberUID);
        ...
        if (subscription) {
            ...
            logger.info(`[unsubscribeBars]: Unsubscribed ${subscriberUID}`);
        }
    }
    ...
    getQuotes?(
        ...
        logger.info('[getQuotes]: Method called', symbols);
        ...
        Promise.allSettled(quotePromises)
            ...
            .catch(error => {
                logger.error('[getQuotes]: Error fetching quotes', error);
