# backtesting/options_backtest.py
...
class OptionsBacktester:
    ...
    def run(self, initial_capital: float = 10000) -> dict:
        ...
        logger.info(
            {
                "message": f"Running options backtest with initial capital={initial_capital:.2f}"
            }
        )
        try:
            ...
            stock_data["signal"] = signals["action"].reindex(
                stock_data.index, fill_value="hold"
            )
            ...
            for idx in positions.index:
                if idx in options_data.index and idx in stock_data.index:
                    ...
                    if positions[idx] == 1:  # Buy
                        ...
                        option_return = (
                            (stock_price - opt.strike) / opt.strike
                            if stock_price > opt.strike
                            else -opt.iv
                        )
                        returns[idx] = option_return * opt.quantity
                    elif positions[idx] == -1:  # Sell
                        option_return = (
                            (opt.strike - stock_price) / opt.strike
                            if stock_price < opt.strike
                            else -opt.iv
                        )
                        returns[idx] = option_return * opt.quantity
            ...
            win_loss_ratio = (
                win_trades / loss_trades if loss_trades > 0 else float("inf")
            )
            ...
            results = {
                ...
                "options_metrics": {
                    "average_delta": options_data["delta"].mean()
                    if "delta" in options_data
                    else 0.0,
                    "average_iv": options_data["iv"].mean()
                    if "iv" in options_data
                    else 0.0,
                    "average_gamma": options_data["gamma"].mean()
                    if "gamma" in options_data
                    else 0.0,
                    "average_theta": options_data["theta"].mean()
                    if "theta" in options_data
                    else 0.0,
                },
            }
            logger.info(
