...
class AgentConfig(BaseSettings):
    ...
    performance_alert_threshold: float = Field(
        5.0, env="PERFORMANCE_ALERT_THRESHOLD", ge=0.1, le=60.0
    )

    # Agent-specific overrides (JSON format)
    agent_overrides: Dict[str, Dict[str, Any]] = Field(default_factory=dict, env="AGENT_OVERRIDES")
...
class ConfigManager:
    ...
    def _validate_environment_config(self, config: GoldenSignalsConfig):
        ...
        if config.environment == Environment.PRODUCTION:
            ...
            assert (
                config.security.secret_key.get_secret_value() != "dev-secret-key"
            ), "Production secret key required"
            assert (
                config.database.password.get_secret_value() != "password"
