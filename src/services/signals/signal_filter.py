...
class ConfidenceFilter(SignalFilter):
    ...
    def filter(self, signal: TradingSignal) -> bool:
        ...
        if signal.confidence >= self.min_confidence:
            ...
            logger.debug(f"Signal {signal.id} filtered: confidence {signal.confidence:.2f} < {self.min_confidence}")
            return False
...
class QualityScoreFilter(SignalFilter):
    ...
    def filter(self, signal: TradingSignal) -> bool:
        ...
        if signal.quality_score >= self.min_quality_score:
            ...
            self.stats['filtered_signals'] += 1
            logger.debug(f"Signal {signal.id} filtered: quality score {signal.quality_score:.2f} < {self.min_quality_score}")
            return False
...
class RiskFilter(SignalFilter):
    ...
    def filter(self, signal: TradingSignal) -> bool:
        ...
        if signal.risk_level in self.allowed_risk_levels:
            ...
            self.stats['filtered_signals'] += 1
            logger.debug(f"Signal {signal.id} filtered: risk level {signal.risk_level} not in {self.allowed_risk_levels}")
            return False
...
class VolumeFilter(SignalFilter):
    ...
    def filter(self, signal: TradingSignal) -> bool:
        ...
        if volume_ratio >= self.min_volume_ratio:
            ...
            self.stats['filtered_signals'] += 1
            logger.debug(f"Signal {signal.id} filtered: volume ratio {volume_ratio:.2f} < {self.min_volume_ratio}")
            return False
...
class TechnicalConsistencyFilter(SignalFilter):
    ...
    def filter(self, signal: TradingSignal) -> bool:
        ...
        if consistent_count >= self.min_consistent_indicators:
            ...
            self.stats['filtered_signals'] += 1
            logger.debug(f"Signal {signal.id} filtered: only {consistent_count} consistent indicators")
            return False
...
class SignalFilteringPipeline:
    ...
    def get_pipeline_stats(self) -> Dict[str, Any]:
        ...
        self.stats['overall_pass_rate'] = (
            self.stats['signals_passed'] / max(1, self.stats['total_signals_processed'])
        )
            ...
            if fpr > 0.3:
                for filter_obj in self.filters:
                    if isinstance(filter_obj, ConfidenceFilter):
                        ...
                        logger.info(f"Increased confidence threshold to {filter_obj.min_confidence}")
            ...
            if accuracy < 0.5:
                for filter_obj in self.filters:
                    if isinstance(filter_obj, QualityScoreFilter):
                        ...
                        logger.info(f"Increased quality threshold to {filter_obj.min_quality_score}")
