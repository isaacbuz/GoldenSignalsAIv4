...
from src.services.market.quality_validator import DataQualityReport, DataQualityValidator
...
class SignalGenerationEngine:
    ...
    async def _generate_signal_for_symbol(self, symbol: str) -> Optional[TradingSignal]:
        ...
        if symbol in self.signal_cache:
            ...
            # Ensure cache_time is timezone-aware
            if cache_time.tzinfo is None:
                cache_time = cache_time.replace(tzinfo=timezone.utc)
            if (now_utc() - cache_time).total_seconds() < self.cache_ttl:
    ...
    async def _calculate_indicators(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        indicators["bb_percent"] = (data["Close"] - indicators["bb_lower"]) / (
    ...
    def _calculate_stochastic_sync(
        self, data: pd.DataFrame, period: int = 14
    ) -> Tuple[pd.Series, pd.Series]:
    ...
    async def _analyze_and_generate_signal(
        ...
        if latest_features.get("macd_cross") == 1:
            ...
            if action != "SELL":
                action = "BUY"
        elif latest_features.get("macd_cross") == 0 and latest_indicators.get(
            "macd", 0
        ...
        signal = TradingSignal(
            ...
            reason="; ".join(reasons),
            indicators={
                k: float(v) if not pd.isna(v) else 0.0 for k, v in latest_indicators.items()
            },
            ...
            metadata={
                ...
                "features": {
                    k: float(v) if not pd.isna(v) else 0.0 for k, v in latest_features.items()
                },
            },
    ...
    def train_ml_model(self, historical_data: pd.DataFrame, labels: pd.Series):
        ...
        try:
            ...
            logger.info(
                f"ML model trained with accuracy: {self.ml_model.score(features_scaled, labels_clean):.2f}"
            )
