...
class LiveDataService:
    ...
    async def _health_check_loop(self):
        ...
        while self.running:
            try:
                ...
                await self.redis_manager.publish_market_data(
                    "system",
                    {
                        "service": "live_data",
                        "status": "healthy",
    ...
    async def _on_quote_update(self, quote: MarketData):
        ...
        try:
            ...
            await self.redis_manager.cache_market_data(
                quote.symbol, asdict(quote), ttl=self.config.cache_ttl
            )
    ...
    async def _on_options_update(self, options: List[OptionsData]):
        ...
        try:
            ...
            await self.redis_manager.cache_options_data(
                symbol, options_data, ttl=self.config.cache_ttl
            )
            ...
            if unusual:
                ...
                await self.redis_manager.publish_signal(
                    symbol,
                    {
        ...
        for symbol in self.config.symbols:
            ...
            if cached_data:
                summary["symbols"][symbol] = {
                    ...
                    "change_percent": (
                        (cached_data.get("price", 0) - cached_data.get("open", 0))
                        / cached_data.get("open", 1)
                        * 100
                    )
                    if cached_data.get("open")
                    else 0,
                ...
                unusual.append(
                    {
                        ...
                        "volume_oi_ratio": opt.volume / opt.open_interest
    ...
    async def get_quote(self, symbol: str) -> Optional[MarketData]:
        ...
        if cached and (
            datetime.now(timezone.utc) - datetime.fromisoformat(cached.get("timestamp", ""))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "uptime": str(datetime.now(timezone.utc) - self.stats["uptime_start"])
