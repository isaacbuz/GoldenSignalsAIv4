    ...
    def session(self):
        if self._session is None:
            ...
            self._session = requests_cache.CachedSession(
                cache_name="yfinance_cache", expire_after=300  # 5 minutes
            )
    ...
    async def fetch_price(self, symbol: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            ticker = await loop.run_in_executor(
                None, lambda: yf.Ticker(symbol, session=self.session)
            )
...
class MarketDataManager:
    ...
    def __init__(self) -> None:
        ...
        self.circuit_breakers = {
            provider.__class__.__name__: CircuitBreaker() for provider in self.providers
        }
    ...
    async def get_historical_data(
        self, symbol: str, period: str = "1mo", interval: str = "1d", use_cache: bool = True
    ) -> pd.DataFrame:
