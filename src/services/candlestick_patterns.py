...
class CandlestickPatternService:
    ...
    def _detect_talib_patterns(self, df: pd.DataFrame) -> List[CandlestickPattern]:
        ...
        talib_patterns = {
            ...
            'CDL3WHITESOLDIERS': (talib.CDL3WHITESOLDIERS, PatternType.THREE_WHITE_SOLDIERS, 'bullish'),
                ...
                for i, val in enumerate(result):
                    if val != 0:  # Pattern detected
                        ...
                        if pattern_name == 'CDLENGULFING':
                            pattern_type = PatternType.BULLISH_ENGULFING if val > 0 else PatternType.BEARISH_ENGULFING
                            direction = 'bullish' if val > 0 else 'bearish'
                        elif pattern_name == 'CDLHARAMI':
                            pattern_type = PatternType.BULLISH_HARAMI if val > 0 else PatternType.BEARISH_HARAMI
                            direction = 'bullish' if val > 0 else 'bearish'
                        elif pattern_name == 'CDL3INSIDE':
                            pattern_type = PatternType.THREE_INSIDE_UP if val > 0 else PatternType.THREE_INSIDE_DOWN
                            direction = 'bullish' if val > 0 else 'bearish'
                        elif pattern_name == 'CDLRISEFALL3METHODS':
                            pattern_type = PatternType.RISING_THREE_METHODS if val > 0 else PatternType.FALLING_THREE_METHODS
                            direction = 'bullish' if val > 0 else 'bearish'
                        
                        if pattern_type:
                            pattern = self._create_pattern(
                                df, i, pattern_type, direction, abs(val)
                            )
        ...
        for i in range(len(df)):
            ...
            if total_range > 0:
                ...
                if body_ratio > 0.95:
                    ...
                    pattern = self._create_pattern(
                        df, i, PatternType.MARUBOZU, direction, 100
                    )
                    patterns.append(pattern)
        ...
        for i in range(1, len(df)):
            ...
            if high_diff < 0.001:  # Within 0.1%
                ...
                if self._is_at_resistance(df, i):
                    pattern = self._create_pattern(
                        df, i, PatternType.TWEEZER_TOP, 'bearish', 80
                    )
                    patterns.append(pattern)
            ...
            if low_diff < 0.001:  # Within 0.1%
                ...
                if self._is_at_support(df, i):
                    pattern = self._create_pattern(
                        df, i, PatternType.TWEEZER_BOTTOM, 'bullish', 80
                    )
                    patterns.append(pattern)
    ...
    def get_pattern_statistics(
        ...
        for pattern in patterns:
            pattern_counts[pattern.pattern_type.value] = pattern_counts.get(pattern.pattern_type.value, 0) + 1
