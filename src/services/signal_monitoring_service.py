...
class SignalMonitoringService:
    ...
    def track_signal_entry(self, signal_data: Dict[str, Any]):
        ...
        logger.info(
            f"Tracking signal entry: {signal_id} - {outcome.symbol} {outcome.action} @ {outcome.entry_price}"
        )

    def track_signal_exit(
        self, signal_id: str, exit_price: float, outcome: str = "success", notes: str = ""
    ):
        ...
        signal_outcome.profit_loss_percent = (
            signal_outcome.profit_loss / signal_outcome.entry_price
        ) * 100
        ...
        logger.info(
            f"Signal exit tracked: {signal_id} - P/L: ${signal_outcome.profit_loss:.2f} ({signal_outcome.profit_loss_percent:.2f}%)"
        )
    ...
    def get_performance_metrics(
        self, timeframe: Optional[timedelta] = None, symbol: Optional[str] = None
    ) -> PerformanceMetrics:
        ...
        if (
            ...
            and (datetime.now(timezone.utc) - self.cache_timestamp).seconds < self.cache_ttl
            and not timeframe
            and not symbol
        ):
        ...
        # Win rate
        completed_count = (
            metrics.successful_signals + metrics.failed_signals + metrics.partial_signals
        )
        if completed_count > 0:
            metrics.win_rate = (
                metrics.successful_signals + metrics.partial_signals * 0.5
            ) / completed_count
        ...
        if signals:
            sorted_by_pnl = sorted(
                [s for s in signals if s.profit_loss is not None], key=lambda x: x.profit_loss
            )
            if sorted_by_pnl:
        ...
        for signal in signals:
            if signal.symbol not in metrics.by_symbol:
                metrics.by_symbol[signal.symbol] = {"count": 0, "win_rate": 0, "total_pnl": 0}
        ...
        for action in ["BUY", "SELL"]:
            ...
            if action_signals:
                ...
                metrics.by_action[action] = {
                    ...
                    "avg_pnl": np.mean(
                        [s.profit_loss for s in action_signals if s.profit_loss is not None]
                    ),
                }
