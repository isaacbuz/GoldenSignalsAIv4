...
class SimplifiedMLSignalGenerator:
    ...
    async def generate_signals(
        self, symbols: Optional[List[str]] = None, limit: int = 10, live_data_fetcher=None
    ) -> List[Dict[str, Any]]:
    ...
    async def _generate_signal_for_symbol(
        self, symbol: str, live_data_fetcher=None
    ...
    def _moving_average_strategy(self, indicators: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if trend == "bullish" and ma_ratio > 1.005:  # 0.5% above
            return {
                ...
                "reasoning": ["Golden cross pattern detected", "20 SMA trending above 50 SMA"],
            }
            ...
            return {
                ...
                "confidence": 70,
                "strategy": "MA_CROSSOVER",
                "reasoning": ["Death cross pattern detected", "20 SMA trending below 50 SMA"],
            }

        return None

    def _volume_strategy(
        self, indicators: Dict[str, Any], data: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
    ...
    def _generate_random_signal(self) -> Dict[str, Any]:
        ...
        reasoning_map = {
            "RSI_MACD": [f"RSI at {random.uniform(25, 75):.1f}", "MACD showing momentum shift"],
            "BOLLINGER_BANDS": ["Price testing Bollinger Band", "Mean reversion opportunity"],
            "MA_CROSSOVER": ["Moving average crossover detected", "Trend confirmation signal"],
            "VOLUME_SURGE": [
    ...
    def _combine_signals(
        ...
        return {
            ...
            "reasoning": list(set(all_reasoning))[:4]
            if all_reasoning
            else ["Technical analysis signal"],
            "strategies": strategies_used if strategies_used else ["TECHNICAL"],
            "indicators": {
                "rsi": round(indicators.get("rsi", 50), 1)
                if not np.isnan(indicators.get("rsi", 50))
                else 50,
                "trend": indicators.get("trend", "neutral"),
