...
class RateLimitHandler:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        self.error_counts: Dict[DataSource, int] = {
            source: 0 for source in DataSource
        }
        self.last_error_time: Dict[DataSource, float] = {
            source: 0 for source in DataSource
        }
        ...
        for i in range(0, len(uncached_symbols), batch_size):
            ...
            for source in self._get_source_priority():
                ...
                if await self._can_make_request(source, len(batch)):
                    try:
                        batch_data = await self._fetch_batch_quotes_from_source(
                            batch, source
                        )
    ...
    def _increment_error_count(self, source: DataSource):
        ...
        self.error_counts[source] = min(
            ...
            self.rate_limits[source].max_retries
        )
