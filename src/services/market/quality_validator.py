...
class DataQualityValidator:
    ...
    async def get_market_data_with_fallback(
        ...
        for i, source_func in enumerate(self.data_sources):
            try:
                logger.info(
                    f"Attempting to fetch {symbol} from source {i+1}/{len(self.data_sources)}"
                )
    ...
    async def _fetch_yahoo_finance(
        ...
        try:
            ...
            data = await loop.run_in_executor(
                None, lambda: ticker.history(period=period, interval="1d")
            )
    ...
    def validate_market_data(self, data: pd.DataFrame, symbol: str) -> DataQualityReport:
        ...
        # Overall validity
        is_valid = (
            completeness >= 0.8 and accuracy >= 0.8 and timeliness >= 0.8 and consistency >= 0.8
        )
    ...
    def _check_completeness(
        self, data: pd.DataFrame, issues: List[str], recommendations: List[str]
    ) -> float:
    ...
    def _check_accuracy(
        ...
        for column in ["Open", "High", "Low", "Close"]:
            if column in data.columns:
                ...
                IQR = Q3 - Q1
                outliers = ((data[column] < (Q1 - 3 * IQR)) | (data[column] > (Q3 + 3 * IQR))).sum()
            ...
            if missing_days > 0:
                ...
                if gap_ratio > 0.1:  # More than 10% missing
                    ...
                    recommendations.append(
                        "Consider using a different data source with complete history"
                    )
    ...
    def detect_outliers(
        ...
        if method == "zscore":
            ...
            Q3 = data[column].quantile(0.75)
            IQR = Q3 - Q1
            outliers = (data[column] < (Q1 - threshold * IQR)) | (
