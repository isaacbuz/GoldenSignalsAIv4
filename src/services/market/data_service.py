...
class MarketDataCache:
    ...
    def save_historical(self, symbol: str, data: pd.DataFrame):
        ...
        try:
            ...
            self.memory_cache[f"hist_{symbol}"] = {"timestamp": datetime.now().timestamp()}
        except Exception as e:
...
class MarketDataService:
    ...
    async def _fetch_quote(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        quote_data = await self.rate_limit_handler.get_quote(
            symbol, priority=RequestPriority.NORMAL
        )
    ...
    async def subscribe_to_symbol(self, symbol: str, callback: Callable):
        ...
        sub_id = await self.websocket_service.subscribe([symbol], callback=callback)
    ...
    async def get_quotes(self, symbols: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            self.monitoring_service.record_latency(
                "batch_quotes", latency_ms, {"count": str(len(symbols))}
            )
