...
class ChartVisionAnalyzer:
    ...
    async def analyze_chart_image(self, 
        ...
        signals = self._generate_trading_signals(
            trend, patterns, support_resistance, indicators
        )
        
        # Create comprehensive analysis
        analysis = self._create_analysis_report(
            chart_type, trend, patterns, support_resistance, 
            indicators, insights, signals, user_context
        )
    ...
    async def _detect_patterns(self, image: np.ndarray, 
                             price_data: List[Dict]) -> List[Dict[str, Any]]:
    ...
    def _generate_insights(self, chart_type: str, trend: str,
                         patterns: List[Dict], support_resistance: List[Dict],
                         indicators: List[Dict]) -> List[str]:
        ...
        for pattern in patterns[:2]:  # Top 2 patterns
            if pattern['type'] == 'head_shoulders':
                insights.append(f"🎯 Head and Shoulders pattern detected - potential reversal at {pattern['neckline']}")
                ...
                insights.append(f"🏔️ Double top pattern at {pattern['price']} - bearish reversal signal")
        
        # Support/Resistance insights
        if support_resistance:
            key_level = support_resistance[0]
            insights.append(f"🎯 Key {key_level['type']} at {key_level['price']:.2f} with {key_level['touches']} touches")
        
        # Indicator insights
        for indicator in indicators:
            if indicator['type'] == 'moving_average':
                insights.append(f"📊 Price {'above' if indicator['position'] == 'above' else 'below'} {indicator['period']} MA")
    ...
    def _generate_trading_signals(self, trend: str, patterns: List[Dict],
        ...
        for level in support_resistance:
            if level['type'] == 'support' and abs(current_price - level['price']) / current_price < 0.02:
                signals.append({
                    'type': 'BUY',
                    'entry': level['price'],
    ...
    def _create_analysis_report(self, chart_type: str, trend: str,
                              patterns: List[Dict], support_resistance: List[Dict],
                              indicators: List[Dict], insights: List[str],
                              signals: List[Dict], user_context: Optional[str]) -> str:
...
"""
    ...
    def _get_exit_analysis(self, patterns: List[Dict], trend: str) -> str:
        ...
        if patterns and any(p['type'] in ['double_top', 'head_shoulders'] for p in patterns):
            return "Warning: Reversal patterns detected. Consider taking profits or tightening stops."
        elif trend == "bullish":
    ...
    def _detect_lower_panel_indicators(self, image: np.ndarray) -> List[Dict]:
        ...
        return [{
            'type': 'rsi',
            'value': 65,
            'description': 'RSI at 65 - moderately bullish'
        }]
...
async def analyze_chart_screenshot(image_data: str, context: Optional[str] = None) -> Dict[str, Any]:
