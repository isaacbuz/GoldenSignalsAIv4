            ...
            (object,),
            {
                "choices": [
                    type(
                        "obj",
                        (object,),
                        {"message": type("obj", (object,), {"content": "Mock AI response"})()},
                    )
                ]
            },
        )()
...
class MultimodalAIChatService:
            ...
            self.openai_client = type(
                ...
                (object,),
                {
                    "chat": type(
                        "obj",
                        (object,),
                        {"completions": type("obj", (object,), {"create": MockOpenAI().create})()},
                    )()
                },
            )()
    ...
    async def process_message(
        self, session_id: str, message: str, attachments: List[UploadFile] = None
    ) -> AIResponse:
        ...
        try:
            ...
            user_msg = ChatMessage(
                session_id=session_id, type=MessageType.TEXT, content=message, role="user"
            )
    ...
    async def _process_attachments(self, attachments: List[UploadFile]) -> List[Dict[str, Any]]:
        ...
        for attachment in attachments:
            try:
                ...
                processed.append({"filename": attachment.filename, "error": str(e)})
    ...
    async def _analyze_financial_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            price_columns = [
                col for col in df.columns if "close" in col.lower() or "price" in col.lower()
            ]
            if price_columns:
                ...
                analysis["returns_stats"] = {
                    ...
                    "sharpe": float(df["returns"].mean() / df["returns"].std() * np.sqrt(252))
                    if df["returns"].std() > 0
                    else 0,
            ...
            if volume_columns:
                ...
                analysis["volume_stats"] = {
                    ...
                    if df[volume_col].iloc[-5:].mean() > df[volume_col].mean()
                    else "decreasing",
                }
    ...
    async def _generate_multimodal_response(
        self, session: ChatSession, user_msg: ChatMessage, context: Dict[str, Any]
    ) -> AIResponse:
