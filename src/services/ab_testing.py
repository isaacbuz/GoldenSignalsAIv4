...
class ABTestingService:
    ...
    def create_experiment(
        ...
        Returns:
            Created experiment
        """
        experiment_id = hashlib.md5(f"{name}_{datetime.now().isoformat()}".encode()).hexdigest()[
            :12
        ]
        ...
        for i, (config, alloc) in enumerate(zip(test_agent_configs, allocation_percentages[1:])):
            variant = ExperimentVariant(
                name=f"variant_{i+1}", agent_config=config, allocation_percentage=alloc
            )
    ...
    def assign_variant(
        self, experiment_id: str, user_id: str, symbol: str
    ...
    def record_signal_result(
        self, experiment_id: str, variant_name: str, metrics: Dict[str, float]
    ):
        """
    ...
    def analyze_experiment(self, experiment_id: str, metric_name: str) -> Dict[str, Any]:
        """
        ...
        for variant in experiment.test_variants:
            ...
            if (
                control_stats["count"] >= experiment.min_sample_size
                and variant_stats["count"] >= experiment.min_sample_size
            ):
                # Perform t-test
                ...
                if control_values and variant_values:
                    ...
                    effect_size = (
                        (variant_stats["mean"] - control_stats["mean"]) / pooled_std
                        if pooled_std > 0
                        else 0
                    )
    ...
    def _generate_recommendation(
        self, results: Dict[str, Any], experiment: ABTestExperiment
    ) -> Dict[str, str]:
        """Generate recommendation based on analysis results"""
        recommendation = {"action": "continue", "reasoning": ""}
        ...
        for variant_name, variant_data in results["variants"].items():
            ...
            if (
                variant_data.get("is_significant", False)
                and variant_data.get("lift_percentage", 0) > best_lift
            ):
        ...
        if best_variant and best_lift > 5:  # 5% improvement threshold
            ...
            v.signal_count >= experiment.min_sample_size * 2 for v in experiment.get_all_variants()
        ):
            recommendation["action"] = "stop"
            recommendation[
                "reasoning"
            ] = "No significant improvement found after sufficient samples"
        else:
    ...
    def get_experiment_report(self, experiment_id: str) -> Dict[str, Any]:
        ...
        # Analyze each metric
        for metric in experiment.target_metrics:
            report["metrics_analysis"][metric] = self.analyze_experiment(experiment_id, metric)
