...
class WebSocketService:
    ...
    async def subscribe(
        self, symbols: List[str], channels: List[str] = None, callback: Optional[Callable] = None
    ) -> str:
        ...
        subscription = Subscription(
            id=sub_id, symbols=symbols, channels=channels, callback=callback, created_at=time.time()
        )
            ...
            try:
                if self.state == ConnectionState.CONNECTED and self.ws:
                    ...
                    if time.time() - self.last_heartbeat > self.heartbeat_interval:
                        message = {"type": MessageType.HEARTBEAT.value, "timestamp": time.time()}
                        await self.ws.send(json.dumps(message))
            ...
            try:
                ...
                if all_symbols:
                    ...
                    quotes = await self.rate_limit_handler.batch_get_quotes(
                        list(all_symbols), priority=RequestPriority.HIGH
                    )
