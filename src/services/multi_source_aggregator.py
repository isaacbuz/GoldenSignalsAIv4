        ...
        try:
            ...
            for sub_name in subreddits:
                ...
                for submission in subreddit.hot(limit=25):
                    if (
                        symbol.upper() in submission.title.upper()
                        or f"${symbol.upper()}" in submission.title
                    ):
                        mentions += 1
...
class MultiSourceAggregator:
    ...
    def _initialize_sources(self) -> Dict[str, DataSource]:
        ...
        # StockTwits
        sources["stocktwits"] = StockTwitsSource(
            SourceConfig(name="StockTwits", api_key=None, rate_limit=200, weight=0.2, cache_ttl=300)
        )

        # FRED
        if os.getenv("FRED_API_KEY"):
            sources["fred"] = FREDSource(
                SourceConfig(
                    name="FRED",
        ...
        if os.getenv("REDDIT_CLIENT_ID"):
            sources["reddit"] = RedditSource(
                SourceConfig(name="Reddit", api_key=None, rate_limit=60, weight=0.2, cache_ttl=600)
            )

        # Discord
        sources["discord"] = DiscordMonitor(
            SourceConfig(name="Discord", api_key=None, rate_limit=100, weight=0.2, cache_ttl=60)
        )
    ...
    async def get_aggregated_sentiment(self, symbol: str) -> MarketSentiment:
        ...
        for name, result in zip(source_names, results):
            ...
            if result and "sentiment" in result:
                ...
                social_mentions += result.get("mention_count", 0) + result.get("message_count", 0)
    ...
    async def get_market_pulse(self, symbols: List[str]) -> Dict[str, MarketSentiment]:
        ...
        return {sentiment.symbol: sentiment for sentiment in results}
...
async def main():
    ...
    print(
        f"AAPL Sentiment: {sentiment.composite_score:.2f} (confidence: {sentiment.confidence:.2f})"
    )
