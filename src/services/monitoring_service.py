...
class MonitoringService:
    ...
    def __init__(self):
        ...
        self.stats = {
            ...
            "errors": {"total": 0, "rate": 0, "by_type": defaultdict(int)},
        }
    ...
    def record_latency(self, operation: str, latency_ms: float, tags: Dict[str, str] = None):
        ...
        metric = MetricPoint(timestamp=time.time(), value=latency_ms, tags=tags)
    ...
    def record_cache_hit(self, cache_level: str, hit: bool):
        ...
        metric = MetricPoint(
            timestamp=time.time(), value=1 if hit else 0, tags={"level": cache_level}
        )
    ...
    def record_error(self, error_type: str, tags: Dict[str, str] = None):
        ...
        metric = MetricPoint(
            timestamp=time.time(), value=1, tags={"type": error_type, **(tags or {})}
        )

        self.metrics[MetricType.ERROR_RATE].append(metric)

    def get_latency_stats(
        self, operation: str = None, window_seconds: int = 300
    ) -> Dict[str, float]:
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "api": {
                ...
                "top_endpoints": dict(
                    sorted(
                        self.stats["api"]["by_endpoint"].items(), key=lambda x: x[1], reverse=True
                    )[:10]
                ),
            },
            "errors": {
                ...
                "top_errors": dict(
                    sorted(
                        self.stats["errors"]["by_type"].items(), key=lambda x: x[1], reverse=True
                    )[:5]
                ),
            },
            "alerts": [
