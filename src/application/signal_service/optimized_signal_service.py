...
class OptimizedSignalService:
    ...
    async def generate_signal(self, 
                            ...
                            priority: bool = False) -> UnifiedSignal:
        """
        ...
        consensus = await self.consensus.request_consensus(
            symbol=symbol,
            context=context,
            timeout=2.0 if priority else 5.0
        )
        
        # Get historical accuracy for this type of signal
        accuracy = await self._get_historical_accuracy(
            symbol, consensus.final_signal.value
        )
    ...
    async def get_recent_signals(self, 
                               ...
                               limit: int = 50,
                               signal_type: Optional[SignalType] = None) -> List[UnifiedSignal]:
        ...
        if self.signal_cache:
            ...
            for signal in self.signal_cache.values():
                ...
                processing_times.append(
                    signal.metadata.get('processing_time_ms', 0)
                )
    ...
    async def _cache_signal(self, signal: UnifiedSignal):
        ...
        if len(self.signal_cache) > 1000:
            ...
            oldest_ids = sorted(
                ...
                key=lambda sid: self.signal_cache[sid].timestamp
            )[:100]
        ...
        await self.redis.lpush(
            f"recent_signals:{signal.symbol}",
            signal.id
        )
    ...
    def _generate_reasoning(self, consensus: ConsensusResult, context: Dict[str, Any]) -> str:
        ...
        if consensus.agreement_score > 0.8:
            ...
            reasoning_parts.append(f"Moderate consensus ({consensus.agreement_score:.0%} agreement)")
        else:
        ...
        for key in ['sentiment', 'technical', 'options_flow', 'risk', 'regime']:
            if key in context:
                if isinstance(context[key], dict):
                    summary[key] = {
                        k: v for k, v in context[key].items()
    ...
    def _calculate_expected_move(self, consensus: ConsensusResult, context: Dict[str, Any]) -> float:
    ...
    async def _update_metrics(self, signal: UnifiedSignal, start_time: datetime):
        ...
        await self.redis.publish("metrics:signals", json.dumps({
            "signal_id": signal.id,
