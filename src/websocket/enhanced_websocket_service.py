...
class EnhancedWebSocketService:
    ...
    async def connect(
        ...
        try:
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(uri, extra_headers=headers), timeout=self.connection_timeout
            )
    ...
    async def _handle_reconnection(self, connection_id: str):
        ...
        delay = min(
            self.reconnect_delay * (2 ** (status.reconnect_attempts - 1)), self.max_reconnect_delay
        )

        logger.info(
            f"Reconnecting {connection_id} in {delay}s (attempt {status.reconnect_attempts})"
        )
    ...
    async def _process_data_buffer(self):
        ...
        while self.running:
            try:
                ...
                update = await asyncio.wait_for(self.data_buffer.get(), timeout=1.0)
    ...
    async def _metrics_reporter(self):
        ...
        while self.running:
            try:
                ...
                latencies = [
                    s.latency_ms for s in self.connection_status.values() if s.latency_ms > 0
                ]
