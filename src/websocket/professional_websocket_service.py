        ...
        if config.rate_limits:
            ...
            if 'per_second' in config.rate_limits:
                self.rate_limiter = RateLimiter(
                    config.rate_limits['per_second'], 1.0
                )
            elif 'per_minute' in config.rate_limits:
                self.rate_limiter = RateLimiter(
                    config.rate_limits['per_minute'], 60.0
                )
    ...
    async def get_quote(self, symbol: str) -> Optional[MarketData]:
        ...
        for source in self.sources:
            try:
                ...
                logger.warning(
                    f"Source {source.config.name.value} failed for {symbol}: {e}"
                )
                continue
        ...
        if self.sources:
            try:
                await self.sources[0].subscribe(
                    symbol, 
                    lambda data: self._handle_update(symbol, data)
                )
            except Exception as e:
        ...
        while self.running:
            for source in self.sources:
                try:
                    ...
                    logger.warning(
                        f"Health check failed for {source.config.name.value}: {e}"
                    )
