...
class MessageRouter:
    ...
    async def route_to_connection(self, connection_id: str, message: Dict[str, Any]) -> bool:
    ...
    async def route_to_symbol_subscribers(self, symbol: str, message: Dict[str, Any]) -> int:
        """Route message to all symbol subscribers across servers"""
        channel = f"symbol:{symbol}"
        message_data = {"symbol": symbol, "message": message, "timestamp": time.time()}

        return await self.redis.publish(channel, json.dumps(message_data))

    async def route_to_user(self, user_id: str, message: Dict[str, Any]) -> int:
        """Route message to all connections of a user across servers"""
        channel = f"user:{user_id}"
        message_data = {"user_id": user_id, "message": message, "timestamp": time.time()}
    ...
    async def broadcast_all(self, message: Dict[str, Any]) -> int:
        ...
        message_data = {"message": message, "timestamp": time.time()}

        return await self.redis.publish(channel, json.dumps(message_data))

    async def register_connection(self, connection_id: str, server_id: str):
        """Register connection location in Redis"""
        await self.redis.hset("connection_servers", connection_id, server_id)
...
class ScalableWebSocketManager:
    ...
    async def unsubscribe_from_symbol(self, connection_id: str, symbol: str) -> bool:
    ...
    async def handle_message(self, connection_id: str, message: str):
        ...
        try:
            ...
            if msg_type == "subscribe":
                ...
                await self._send_to_connection(
                    connection, {"type": "pong", "timestamp": datetime.utcnow().isoformat()}
                )

            elif msg_type == "broadcast":
                # Handle user broadcasts (if authorized)
                if connection.user_id and data.get("message"):
                    await self.broadcast_message(data["message"], sender_id=connection.user_id)
    ...
    async def broadcast_message(
        ...
        if target_type == "symbol" and target_id:
            ...
            return await self.router.route_to_symbol_subscribers(target_id, message_data)
        elif target_type == "user" and target_id:
            # Broadcast to user connections
            return await self.router.route_to_user(target_id, message_data)
        else:
            # Broadcast to all
    ...
    async def send_signal(self, symbol: str, signal_data: Dict[str, Any]):
    ...
    async def _send_to_connection(
        self, connection: ScalableConnection, message: Dict[str, Any]
    ) -> bool:
                ...
                if message and message["type"] == "message":
                    ...
                    if channel == f"server:{self.server_id}":
                        ...
                        if target_conn in self.connections:
                            await self._send_to_connection(
                                self.connections[target_conn], data["message"]
                            )

                    # Handle broadcasts
                    elif channel == "broadcast:all":
                        for connection in self.connections.values():
                            await self._send_to_connection(connection, data["message"])
                    ...
                    # Handle symbol messages
                        ...
                        for conn_id in self.symbol_subscribers.get(symbol, []):
                            if conn_id in self.connections:
                                await self._send_to_connection(
                                    self.connections[conn_id], data["message"]
                                )
    ...
    async def _cleanup_loop(self):
        ...
        while True:
            try:
                ...
                empty_symbols = [
                    symbol for symbol, subs in self.symbol_subscribers.items() if not subs
                ]
                ...
                # Log metrics
                logger.info(
                    f"Server {self.server_id} - Active connections: {len(self.connections)}, "
    ...
    async def _register_server(self):
        ...
        await self.redis.hset("websocket_servers", self.server_id, json.dumps(server_info))

        await self._update_server_heartbeat()

    async def _update_server_heartbeat(self):
        """Update server heartbeat in Redis"""
        await self.redis.hset("server_heartbeats", self.server_id, time.time())
    ...
    async def _save_connection_info(self, connection: ScalableConnection):
        ...
        await self.redis.hset(
            f"connection:{connection.connection_id}", mapping=connection.to_dict()
        )
...
async def demo_scalable_websocket():
    ...
    manager = ScalableWebSocketManager(
        redis_url="redis://localhost:6379", server_id="demo_server_1"
    )
    ...
    await manager.send_market_data("AAPL", {"price": 185.50, "volume": 45000000, "change": 2.3})

    # Send signal
    print("\n🚦 Broadcasting Signal:")
    await manager.send_signal("AAPL", {"type": "BUY", "confidence": 0.85, "price_target": 190.0})

    # Send user alert
    print("\n🔔 Sending User Alert:")
    await manager.send_alert("user123", {"level": "info", "message": "Your AAPL position is up 5%"})
