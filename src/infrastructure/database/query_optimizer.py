...
class DatabaseOptimizer:
    ...
    async def batch_insert(
        self, table: str, columns: List[str], values: List[Tuple], chunk_size: int = 1000
    ) -> int:
        ...
        async with self.acquire_write() as conn:
            ...
            for i in range(0, len(values), chunk_size):
                ...
                result = await conn.copy_records_to_table(table, records=chunk, columns=columns)
    ...
    async def bulk_update(
        self, table: str, updates: List[Dict[str, Any]], key_column: str = "id"
    ) -> int:
        ...
        async with self.acquire_write() as conn:
            async with conn.transaction():
                ...
                await conn.copy_records_to_table(
                    temp_table, records=[tuple(u.values()) for u in updates], columns=columns
                )

                # Perform bulk update
                set_clause = ", ".join(
                    f"{col} = {temp_table}.{col}::TEXT" for col in columns if col != key_column
                )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        if self.enable_query_cache:
            total_cache_requests = metrics["cache_hits"] + (
                metrics["total_queries"] - metrics["cache_hits"]
            )
