...
class BenchmarkSuite:
    ...
    def _print_summary(self):
        ...
        print(f"{'=' * 80}")
        print(
            f"{'Name':<30} {'Category':<15} {'Ops/sec':>12} {'Avg Time (ms)':>15} {'Memory (MB)':>12}"
        )
        print(f"{'-' * 80}")

        for result in self.results:
            print(
                f"{result.name:<30} {result.category:<15} "
...
class CacheBenchmark(PerformanceBenchmark):
    ...
    def __init__(self, cache_manager, data_size: int = 1000):
        super().__init__(
            name=f"Cache_Operations_{data_size}", category="Cache", benchmark_iterations=10000
        )
...
class DatabaseBenchmark(PerformanceBenchmark):
    ...
    def __init__(self, db_optimizer, query_type: str = "simple"):
        super().__init__(
            name=f"Database_{query_type}", category="Database", benchmark_iterations=1000
        )
...
class DataProcessingBenchmark(PerformanceBenchmark):
    ...
    def __init__(self, data_size: int = 10000):
        super().__init__(
            name=f"Data_Processing_{data_size}", category="Processing", benchmark_iterations=100
        )
        self.data_size = data_size
        self.df = None
    ...
    async def run_iteration(self):
        ...
        summary = self.df.groupby("signal").agg(
            {"price": ["mean", "std"], "volume": "sum", "rsi": "mean"}
        )
...
class NetworkBenchmark(PerformanceBenchmark):
    ...
    def __init__(self, endpoint_type: str = "rest"):
        super().__init__(
            name=f"Network_{endpoint_type}", category="Network", benchmark_iterations=500
        )
...
def generate_performance_report(results: List[BenchmarkResult]) -> Dict[str, Any]:
    ...
    report["top_performers"] = [
        {"name": r.name, "ops_per_sec": r.ops_per_second} for r in sorted_by_ops[:5]
    ]

    # Find bottlenecks
    sorted_by_time = sorted(results, key=lambda r: r.avg_time, reverse=True)
    report["bottlenecks"] = [
        {"name": r.name, "avg_time_ms": r.avg_time * 1000} for r in sorted_by_time[:5]
    ]
