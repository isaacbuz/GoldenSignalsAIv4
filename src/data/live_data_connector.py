...
class LiveDataConnector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.engine = create_async_engine(
                self.database_url, echo=False, pool_pre_ping=True, pool_size=20, max_overflow=40
            )
    ...
    async def fetch_historical_data(
        ...
        try:
            ...
            data = ticker.history(
                start=start_date, end=end_date, interval=interval, auto_adjust=True, prepost=True
            )

            if data.empty and self.alpha_vantage_key:
                # Fallback to Alpha Vantage
                logger.info(f"Falling back to Alpha Vantage for {symbol}")
                data, _ = self.alpha_vantage_ts.get_daily_adjusted(symbol=symbol, outputsize="full")
    ...
    async def fetch_technical_indicators(self, symbol: str, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features = {
                "technical_indicators": {
                    "sma": {
                        "20": data["SMA_20"].iloc[-1] if not data["SMA_20"].isna().all() else None
                    },
                    "ema": {
                        "12": data["EMA_12"].iloc[-1] if not data["EMA_12"].isna().all() else None
                    },
                    "rsi": data["RSI"].iloc[-1] if not data["RSI"].isna().all() else None,
                    "macd": {
                        "value": data["MACD"].iloc[-1] if not data["MACD"].isna().all() else None,
                        "signal": data["MACD_signal"].iloc[-1]
                        if not data["MACD_signal"].isna().all()
                        else None,
                    },
                }
            }
            ...
            for symbol in symbols:
                ...
                if self.redis_client:
                    await self.redis_client.setex(
                        f"quote:{symbol}", 60, str(quotes[symbol])  # 1 minute TTL
                    )
    ...
    async def prepare_training_dataset(
        self, symbols: List[str], years: int = 20, features: List[str] = None
    ) -> pd.DataFrame:
    ...
    async def _cache_data(self, symbol: str, data: pd.DataFrame):
        ...
        try:
            ...
            await self.redis_client.setex(f"historical:{symbol}", 3600, str(data_dict))
        except Exception as e:
    ...
    async def _save_training_dataset(self, data: pd.DataFrame):
        ...
        if self.redis_client:
            await self.redis_client.set("training_dataset_metadata", str(metadata))
