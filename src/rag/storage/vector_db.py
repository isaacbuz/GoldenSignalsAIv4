...
class VectorStore(ABC):
    ...
    async def add_vectors(
        self, vectors: List[np.ndarray], metadata: List[Dict[str, Any]]
    ) -> List[str]:
...
class ChromaDBStore(VectorStore):
    ...
    async def add_vectors(
        ...
        self.collection.add(embeddings=[v.tolist() for v in vectors], metadatas=metadata, ids=ids)
    ...
    async def search(
        ...
        """Search ChromaDB"""
        results = self.collection.query(query_embeddings=[query_vector.tolist()], n_results=k)
...
class VectorDBManager:
    ...
    async def similarity_search(
        self, query_embedding: np.ndarray, k: int = 5, filter_type: Optional[str] = None
    ):
