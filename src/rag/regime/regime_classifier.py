...
class RegimeClassifier:
    ...
    def _calculate_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        indicators["volatility"] = (
            talib.ATR(data["high"].values, data["low"].values, data["close"].values, timeperiod=14)[
                -1
            ]
            / data["close"].iloc[-1]
        )
    ...
    def _score_regime(
        self, indicators: Dict[str, float], target_characteristics: Dict[str, float]
    ) -> float:
    ...
    def _detect_regime_start(self, data: pd.DataFrame, regime_type: str) -> datetime:
        ...
        for i in range(lookback, 0, -1):
            ...
            score = self._score_regime(
                indicators, self.regime_definitions[regime_type]["characteristics"]
            )
    ...
    async def predict_regime_change(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        current_score = self._score_regime(
            indicators, self.regime_definitions[current.regime_type]["characteristics"]
        )
    ...
    def _identify_change_triggers(
        self, indicators: Dict[str, float], current_regime: MarketRegime
    ) -> List[str]:
