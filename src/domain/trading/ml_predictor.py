...
class MLPredictor:
    ...
    def prepare_features(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            for i in range(len(close_prices) - self.lookback_period - self.prediction_horizon):
                ...
                # Future return (label)
                future_return = (
                    close_prices[i + self.lookback_period + self.prediction_horizon]
                    - close_prices[i + self.lookback_period]
                ) / close_prices[i + self.lookback_period]
    ...
    def train_models(self, data: pd.DataFrame, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            # Store models
            self.models[symbol] = {"xgb": xgb_model, "lgb": lgb_model, "nn": nn_model}
