...
class ComprehensiveBacktestEngine:
    ...
    def run_backtest(
        self, signal_generator: Callable, price_data: Optional[Dict[str, pd.DataFrame]] = None
    ) -> BacktestResults:
        """
            ...
            for symbol, data in market_data.items():
                ...
                try:
                    ...
                    if signal and signal.get("confidence", 0) >= self.config.confidence_threshold:
                        # Execute trade
                        trade = portfolio.execute_trade(signal, current_date, market_data[symbol])
    ...
    def _fetch_all_market_data(self) -> Dict[str, pd.DataFrame]:
        ...
        try:
            benchmark = yf.download(
                "SPY", start=self.config.start_date, end=self.config.end_date, progress=False
            )
    ...
    def _fetch_symbol_data(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            data = yf.download(
                symbol, start=self.config.start_date, end=self.config.end_date, progress=False
            )
    ...
    def _create_backtest_results(
        self, portfolio: "Portfolio", trades: List[TradeResult]
    ) -> BacktestResults:
        ...
        sortino_ratio = (
            (annualized_return - 0.02) / downside_deviation if downside_deviation > 0 else 0
        )
...
class Portfolio:
    ...
    def execute_trade(
        self, signal: Dict[str, Any], date: datetime, price_data: pd.DataFrame
    ...
    def check_exits(
        self, date: datetime, market_data: Dict[str, pd.DataFrame]
    ) -> List[TradeResult]:
    ...
    def _close_position(
        self, symbol: str, date: datetime, exit_price: float, exit_reason: str
    ) -> TradeResult:
        """Close a position"""
