...
class OnlineLearner:
    ...
    def __init__(self, learning_rate: float = 0.01):
        ...
        self.direction_model = SGDClassifier(
            loss="log_loss", learning_rate="adaptive", eta0=learning_rate, random_state=42
        )

        self.magnitude_model = SGDRegressor(
            loss="squared_error", learning_rate="adaptive", eta0=learning_rate, random_state=42
        )
    ...
    def partial_fit(
        ...
        if not self.is_initialized:
            ...
            self.direction_model.partial_fit(
                scaled_features, directions, classes=[-1, 1], sample_weight=sample_weight
            )
            self.magnitude_model.partial_fit(scaled_features, returns, sample_weight=sample_weight)
            ...
            self.direction_model.partial_fit(
                scaled_features, directions, sample_weight=sample_weight
            )
            self.magnitude_model.partial_fit(scaled_features, returns, sample_weight=sample_weight)
...
class AdaptiveAgent(ABC):
    ...
    def __init__(
        self, agent_id: str, initial_capital: float = 100000, learning_config: Dict[str, Any] = None
    ):
        ...
        # Online learner
        self.learner = OnlineLearner(learning_rate=self.learning_config.get("learning_rate", 0.01))
    ...
    async def make_decision(
        self, market_data: pd.DataFrame, symbol: str, current_position: Optional[Dict] = None
    ) -> TradingDecision:
        """
        ...
        if np.random.random() < self.exploration_rate and not self.is_control_group:
            ...
            if self.learner.is_initialized and not self.is_control_group:
                ...
                if confidence > self.confidence_threshold:
                    ...
                    reasoning = (
                        f"Base signal (low ML confidence): {base_signals.get('reasoning', '')}"
                    )
            else:
                # Use base signals
                action = base_signals.get("action", "HOLD")
    ...
    def calculate_performance_metrics(self) -> AgentPerformanceMetrics:
        ...
        correct_directions = sum(
            1
            for o in self.outcome_buffer
            if (o.decision.action == "BUY" and o.actual_return > 0)
            or (o.decision.action == "SELL" and o.actual_return < 0)
        )
        ...
        if len(predictions) > 10:
            ...
            model_stability = 1 - np.std(recent_predictions) / (
                np.mean(np.abs(recent_predictions)) + 1e-6
            )
        else:
            model_stability = 0.5
...
class AgentPerformanceTracker:
    ...
    def analyze_ab_test(self, test_name: str) -> Dict[str, Any]:
        ...
        improvement = {
            ...
            / (control_metrics.sharpe_ratio + 1e-6),
...
class RSIAdaptiveAgent(AdaptiveAgent):
    ...
    def extract_features(self, market_data: pd.DataFrame, symbol: str) -> Dict[str, float]:
        ...
        return {
            ...
            "price_position": (close.iloc[-1] - close.rolling(20).min().iloc[-1])
