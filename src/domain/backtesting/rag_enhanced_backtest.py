...
class RAGEnhancedBacktestSystem:
    ...
    async def get_market_context(
        self, symbol: str, date: datetime, market_data: pd.DataFrame
    ) -> MarketContext:
        """
        ...
        similar_periods = await self.vector_store.similarity_search(historical_query, k=5)
    ...
    async def _get_news_sentiment(self, query: str) -> Dict:
        ...
        return {
            ...
            "events": [{"type": "earnings", "impact": "positive", "confidence": 0.75}],
        }
    ...
    async def enhance_trading_decision(self, base_decision: Dict, context: MarketContext) -> Dict:
        """
        ...
        if context.similar_historical_periods:
            ...
            successful_periods = [
                p
                for p in context.similar_historical_periods
                if p.get("metadata", {}).get("market_return", 0) > 0
            ]
            historical_success_rate = len(successful_periods) / len(
                context.similar_historical_periods
            )
    ...
    def generate_rag_insights_report(
        self, backtest_results: Dict, contexts: List[MarketContext]
    ) -> Dict:
        """
        ...
        if all_similar_periods:
            report["pattern_matches"] = {
                ...
                "unique_periods": len(
                    set(p.get("metadata", {}).get("date", "") for p in all_similar_periods)
                ),
...
async def demo_rag_benefits():
    ...
    base_decision = {"action": "BUY", "confidence": 0.7, "reasoning": "RSI oversold", "size": 100}
    ...
    sample_results = {"trades": [base_decision, enhanced_decision, enhanced_crisis]}

    insights_report = rag_system.generate_rag_insights_report(
        sample_results, [context_normal, context_crisis]
    )
...
class RAGIntegratedBacktest:
    ...
    async def run_rag_enhanced_backtest(
        ...
        # Fetch data
        market_data = await self.data_manager.fetch_data(symbol, start_date, end_date)

        results = {
            "trades": [],
            "rag_benefits": {"avoided_losses": 0, "improved_timing": 0, "risk_warnings": 0},
        }

        # For each trading day
        for date in market_data.index[50:]:  # Skip warmup
            # Get RAG context
            context = await self.rag_system.get_market_context(symbol, date, market_data[:date])
            ...
            # Track RAG benefits
            if (
                context.regime_classification == "crisis"
                and enhanced_decision["confidence"] < base_decision["confidence"]
            ):
