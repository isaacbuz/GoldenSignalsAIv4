...
class BacktestDataManager:
    ...
    async def fetch_market_data(
        self, symbols: List[str], start_date: datetime, end_date: datetime, interval: str = "5m"
    ...
    async def _fetch_symbol_data(
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                rows = await conn.fetch(query, symbol, start_date, end_date, interval)
    ...
    async def _save_to_redis(self, key: str, df: pd.DataFrame):
        ...
        try:
            # Serialize to JSON
            data_dict = {"data": df.to_dict(), "index": df.index.tolist()}
            self.redis_client.setex(key, self.cache_ttl, json.dumps(data_dict, default=str))
        except Exception as e:
    ...
    async def preload_data(
        self, symbols: List[str], start_date: datetime, end_date: datetime, intervals: List[str]
    ):
