...
# Fix pandas_ta numpy compatibility
import sys
...
# Local imports
from src.utils.timezone_utils import ensure_timezone_aware, now_utc
...
class AdvancedBacktestEngine:
    ...
    async def run_backtest(
        ...
        **kwargs,
        ...
        try:
            ...
            trades = await self._run_backtest_loop(run_id, strategy, market_data, **kwargs)
    ...
    async def _fetch_market_data(
        ...
        for symbol in symbols:
            ...
            data = await simple_live_data.fetch_historical_data(symbol, start_date, end_date, "5m")
            ...
            if data:
                ...
                if self.redis_client:
                    await self.redis_client.setex(cache_key, 3600, df.to_json())  # 1 hour TTL
            ...
            if len(open_positions) < self.config["max_positions"]:
                ...
                for signal in signals:
                    if signal.confidence >= self.config["confidence_threshold"]:
                        position_size = self._calculate_position_size(signal, cash, portfolio_value)
                        ...
                        if position_size > 0:
                            ...
                            cash -= (
                                position_size * signal.entry_price * (1 + self.config["commission"])
                            )
    ...
    async def _generate_signals(
        ...
        for symbol in available_symbols:
            ...
            # Combine signals
            if agent_signals:
                combined_signal = await self._combine_signals(symbol, agent_signals, recent_data)
                if combined_signal:
    ...
    async def _get_agent_signal(
        self, agent_name: str, symbol: str, data: pd.DataFrame
    ...
    async def _get_mcp_signals(self, symbol: str, data: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
    ...
    async def _combine_signals(
        self, symbol: str, agent_signals: Dict[str, Dict[str, Any]], data: pd.DataFrame
    ...
    def _calculate_position_size(
        self, signal: BacktestSignal, cash: float, portfolio_value: float
    ) -> float:
        ...
        if self.config["position_sizing"] == "fixed":
            ...
            win_loss_ratio = (signal.take_profit - signal.entry_price) / (
    ...
    async def _check_exit_conditions(
        self, position: BacktestTrade, current_price: float, current_time: datetime
    ) -> Optional[str]:
    ...
    async def _close_position(
        self, position: BacktestTrade, exit_price: float, exit_time: datetime, exit_reason: str
    ) -> BacktestTrade:
        ...
        position.pnl_percent = position.pnl / (
            position.signal.entry_price * position.signal.position_size
        )

        return position

    async def _calculate_metrics(
        self, trades: List[BacktestTrade], market_data: Dict[str, pd.DataFrame]
    ) -> BacktestMetrics:
        ...
        metrics = BacktestMetrics(
            ...
            avg_loss=avg_loss,
            profit_factor=abs(
                sum(t.pnl for t in winning_trades) / sum(t.pnl for t in losing_trades)
            )
            if losing_trades
            else 0,
            ...
            avg_trade_duration=timedelta(
                seconds=np.mean(
                    [(t.exit_time - t.entry_time).total_seconds() for t in closed_trades]
                )
            )
            if closed_trades
            else timedelta(0),
    ...
    async def _run_monte_carlo(self, trades: List[BacktestTrade]) -> Dict[str, Any]:
        ...
        return {
            ...
            "probability_of_profit": sum(1 for r in simulation_results if r > 0)
            / len(simulation_results),
            "probability_of_loss": sum(1 for r in simulation_results if r < -0.1)
            / len(simulation_results),
        }

    async def _run_walk_forward(
        self, strategy: Union[str, Callable], market_data: Dict[str, pd.DataFrame], **kwargs
    ) -> Dict[str, Any]:
    ...
    async def _run_parameter_optimization(
        ...
        return {"optimal_parameters": {}, "sensitivity_analysis": {}}

    async def _store_results(
        self, run_id: str, metrics: BacktestMetrics, trades: List[BacktestTrade]
    ):
...
async def main():
    ...
    parser.add_argument(
        "--symbols", type=str, nargs="+", default=["AAPL", "MSFT"], help="Symbols to test"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--config", type=str, default="config/backtest_config.yaml", help="Config file"
    )
