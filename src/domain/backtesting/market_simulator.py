...
class MarketMicrostructureSimulator:
    ...
    def simulate_order_book(
        self, symbol: str, mid_price: float, volume: float, volatility: float, timestamp: datetime
    ) -> OrderBook:
        """
        ...
        for i in range(self.book_depth):
            ...
            bids.append(
                OrderBookLevel(
                    price=round(bid_price, 2), quantity=round(bid_size, 0), orders=num_orders
                )
            )

            asks.append(
                OrderBookLevel(
                    price=round(ask_price, 2), quantity=round(ask_size, 0), orders=num_orders
                )
            )
    ...
    def estimate_market_impact(
        self, order: Order, order_book: OrderBook, avg_daily_volume: float
    ) -> float:
        """
    ...
    async def execute_order(
        self, order: Order, order_book: OrderBook, market_data: Dict[str, Any]
    ) -> Order:
        """
    ...
    async def _execute_market_order(
        ...
        impact_pct = self.estimate_market_impact(
            order, order_book, market_data.get("avg_daily_volume", 1000000)
        )
    ...
    def simulate_queue_position(
        self, order: Order, order_book: OrderBook, same_price_orders: int
    ) -> float:
        """
    ...
    def get_execution_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "partial_fills": len(
                [o for o in self.order_history if o.status == OrderStatus.PARTIAL]
            ),
            "rejected_orders": len(
...
async def demo_market_simulator():
    ...
    simulator = MarketMicrostructureSimulator(
        {"base_spread_bps": 2, "market_impact_factor": 0.1, "latency_ms": 50}
    )

    # Simulate order book
    order_book = simulator.simulate_order_book(
        symbol="AAPL", mid_price=150.00, volume=1000000, volatility=0.02, timestamp=datetime.now()
    )
    ...
    market_data = {"avg_daily_volume": 50000000, "volatility": 0.02}
