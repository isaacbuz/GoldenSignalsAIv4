...
class YahooFinanceSource(DataSourceInterface):
    ...
    def validate_data(self, data: pd.DataFrame) -> DataQualityMetrics:
        ...
        # Check completeness
        expected_rows = len(pd.date_range(start=data.index[0], end=data.index[-1], freq="D"))
...
class EnhancedBacktestDataManager:
    ...
    def _initialize_sources(self):
        ...
        if self.config.get("database"):
            self.sources[DataSource.LOCAL_DB] = LocalDatabaseSource(self.config["database"])
    ...
    async def fetch_historical_data(
        ...
        for symbol in symbols:
            task = self._fetch_symbol_with_failover(symbol, start, end, interval, quality_threshold)
            tasks.append(task)
    ...
    async def _fetch_symbol_with_failover(
        self, symbol: str, start: datetime, end: datetime, interval: str, quality_threshold: float
    ) -> Optional[pd.DataFrame]:
        ...
        for source_type in DataSource:
            ...
            try:
                ...
                if quality.overall_score >= quality_threshold:
                    ...
                    if source_type != DataSource.LOCAL_DB and DataSource.LOCAL_DB in self.sources:
...
async def demo():
    ...
    data = await manager.fetch_historical_data(symbols, start, end, interval="1d")
