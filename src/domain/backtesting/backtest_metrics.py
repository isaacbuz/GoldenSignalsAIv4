...
class MetricsCalculator:
    ...
    def calculate_metrics(
        ...
        metrics = BacktestMetrics(
            ...
            calmar_ratio=self._calculate_calmar_ratio(
                self._annualize_return(returns), risk_metrics["max_drawdown"]
            ),
    ...
    def _calculate_trade_statistics(self, trades: List[BacktestTrade]) -> Dict:
        ...
        return {
            ...
            "expectancy": (avg_win * num_wins - avg_loss * num_losses) / total_trades
            if total_trades > 0
            else 0,
    ...
    def _calculate_risk_metrics(self, returns: pd.Series, equity_curve: pd.Series) -> Dict:
    ...
    def _calculate_sortino_ratio(self, returns: pd.Series) -> float:
        ...
        excess_returns = returns - self.risk_free_rate / self.trading_days_per_year
        downside_vol = self._calculate_downside_volatility(returns) / np.sqrt(
    ...
    def _calculate_exposure_time(
        self, trades: List[BacktestTrade], equity_curve: pd.Series
    ) -> float:
        ...
        time_in_market = sum(t.time_in_trade.total_seconds() for t in trades)
