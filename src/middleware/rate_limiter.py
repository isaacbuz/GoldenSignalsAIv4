...
class RateLimiter:
    ...
    def _get_identifier(self, request: Request) -> str:
        ...
        # Use X-Real-IP if behind a trusted proxy, otherwise use direct connection
        client_ip = None

        # Only trust these headers if we're behind a known proxy
        # This should be configured based on your deployment
        trusted_proxy = request.headers.get("X-Proxy-Auth") == "trusted-proxy-secret"

        if trusted_proxy:
            # Try X-Real-IP first (set by nginx/proxy)
            client_ip = request.headers.get("X-Real-IP")
            if not client_ip:
                # Fallback to X-Forwarded-For (rightmost IP before proxy)
                forwarded_for = request.headers.get("X-Forwarded-For")
                if forwarded_for:
                    # Get the rightmost IP that's not a private IP
        ...
        # Use direct connection if no proxy headers or untrusted
        if not client_ip:
    ...
    async def check_rate_limit(
        self, request: Request, limit: int, window_seconds: int
        ...
        try:
            ...
            # Fail closed - deny request if Redis fails for security
            return False, {
                "limit": limit,
                "remaining": 0,
                "reset": int(now + window_seconds),
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    DEFAULT_LIMITS = {
        "/api/v1/signals/generate": (60, 60),  # 60 requests per minute
        "/api/v1/signals": (300, 60),  # 300 requests per minute
        "/api/v1/market-data": (600, 60),  # 600 requests per minute
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Check rate limit
        allowed, metadata = await self.rate_limiter.check_rate_limit(request, limit, window)

        if not allowed:
            # Check if it's a service error vs actual rate limit
            if metadata.get("error"):
                ...
                # Actual rate limit exceeded - return 429
                return JSONResponse(
