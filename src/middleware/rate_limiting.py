...
class RateLimiter:
    ...
    def check_rate_limit(
        self, client_id: str, endpoint: str, tier: str = "public"
    ) -> Dict[str, Any]:
    ...
    def get_usage_stats(self, client_id: str) -> Dict[str, Any]:
        ...
        for key in self.redis.scan_iter(match=pattern):
            ...
            if len(parts) >= 4:
                ...
                stats[endpoint] = {"count": int(count) if count else 0, "ttl": ttl}
...
def create_rate_limit_decorator(
    requests: int = 10, window: int = 60, key_func: Optional[Callable] = None
):
