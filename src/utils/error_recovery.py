...
class ErrorRecoveryService:
        ...
        while True:
            try:
                ...
                if circuit_breaker_config:
                    return await self.circuit_breakers[func_name].call_async(func, *args, **kwargs)
                else:
    ...
    def _record_error(self, error_context: ErrorContext):
        ...
        if error_context.severity == ErrorSeverity.CRITICAL:
            logger.critical(
                f"Critical error in {error_context.function_name}: {error_context.error}"
            )
        elif error_context.severity == ErrorSeverity.HIGH:
            logger.error(
                f"High severity error in {error_context.function_name}: {error_context.error}"
            )
        else:
