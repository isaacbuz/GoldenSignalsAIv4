...
class CVaROptimizer:
    ...
    def optimize(self, target_return: Optional[float] = None) -> PortfolioWeights:
        ...
        constraints = [u >= 0, u >= -portfolio_returns - z, cp.sum(weights) == 1, weights >= 0]
...
class BlackLittermanOptimizer:
    ...
    def optimize(self, views: pd.DataFrame, view_confidence: np.ndarray) -> PortfolioWeights:
        ...
        posterior_cov = np.linalg.inv(tau_sigma_inv + P.T @ np.linalg.inv(omega) @ P)

        # Posterior returns
        posterior_returns = posterior_cov @ (
            tau_sigma_inv @ self.equilibrium_returns + P.T @ np.linalg.inv(omega) @ Q
        )
        ...
        optimizer.cov_matrix = pd.DataFrame(posterior_cov, index=self.assets, columns=self.assets)
...
class KellyOptimizer:
    ...
    def calculate_kelly_fraction(
        self, win_prob: float, win_return: float, loss_return: float
    ) -> float:
        """
    ...
    def optimize_multi_asset(
        self, expected_returns: pd.Series, cov_matrix: pd.DataFrame, confidence: float = 0.25
    ) -> PortfolioWeights:
        """
...
if __name__ == "__main__":
    ...
    returns = pd.DataFrame(
        np.random.multivariate_normal(
            mean=[0.0002] * n_assets, cov=np.eye(n_assets) * 0.01, size=n_days
        ),
