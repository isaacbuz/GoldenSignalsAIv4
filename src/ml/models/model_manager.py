...
class ModelManager:
    ...
    @handle_errors
    def predict(self, model_id: str, X: pd.DataFrame, probability: bool = False) -> np.ndarray:
    ...
    def evaluate_model(
        ...
        if hasattr(model, "predict_proba"):
            ...
            logger.info(
                "Classification metrics for model %s: accuracy=%.3f", model_id, metrics["accuracy"]
            )
        else:
            ...
            logger.info(
                "Regression metrics for model %s: rmse=%.3f, r2=%.3f",
                model_id,
                metrics["rmse"],
                metrics["r2"],
            )
