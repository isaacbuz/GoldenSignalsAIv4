...
async def get_sentiment_analysis(symbol: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        sentiment["visualization"] = {
            ...
            "divergence_rate": sum(
                1
                for data in sentiment["agent_sentiments"].values()
                if data["current"].get("divergence", False)
            )
            / len(sentiment["agent_sentiments"])
            if sentiment["agent_sentiments"]
            else 0,
        }
...
async def update_performance(agent_name: str, signal_id: str, outcome: float) -> Dict[str, Any]:
...
async def get_divergence_analysis(
    ...
    try:
        ...
        divergence_data["symbols_with_divergences"].sort(key=lambda x: x["count"], reverse=True)
...
def _calculate_system_health(dashboard: Dict[str, Any]) -> Dict[str, Any]:
    ...
    avg_accuracy = (
        sum(agent["performance"]["overall"]["accuracy"] for agent in dashboard["agents"].values())
...
def _generate_suggestions(dashboard: Dict[str, Any]) -> List[str]:
    ...
    if dashboard["market_performance"]["average_divergence_rate"] > 0.4:
        suggestions.append(
            "High divergence rate detected. Consider reviewing agent correlations and data sources."
        )
    ...
    for agent_name, data in dashboard["agents"].items():
        if data["performance"]["overall"]["accuracy"] < 0.4:
            suggestions.append(
                f"{agent_name} showing low accuracy. Consider retraining or adjusting parameters."
            )
    ...
    if not suggestions:
        suggestions.append(
            "System performing well. Continue monitoring for optimization opportunities."
        )
    ...
    try:
        ...
        while True:
            ...
            for symbol in orch.symbols[:3]:  # Limit for performance
                signal = orch.generate_signals_for_symbol(symbol)
                await websocket.send_json({"type": "signal_update", "data": signal})

            # Send sentiment update
            sentiment = orch.get_sentiment_analysis()
            await websocket.send_json(
                {"type": "sentiment_update", "data": sentiment["market_sentiment"]}
            )
