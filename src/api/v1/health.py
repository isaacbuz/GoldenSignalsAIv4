...
async def detailed_health_check(
    db=Depends(get_db), redis_client=Depends(get_redis)
) -> Dict[str, Any]:
    ...
    try:
        ...
        health_status["components"]["market_data_api"] = {"status": "unhealthy", "error": str(e)}
    ...
    try:
        ...
        for agent_name, agent in orchestrator.agents.items():
            try:
                ...
                agent_status[agent_name] = {"status": "healthy", "initialized": True}
            except Exception as e:
                agent_status[agent_name] = {"status": "unhealthy", "error": str(e)}

        return {
            "status": "healthy"
            if all(a["status"] == "healthy" for a in agent_status.values())
            else "degraded",
            "timestamp": datetime.utcnow().isoformat(),
            "agents": agent_status,
        }
...
@router.get("/ready")
async def readiness_check(db=Depends(get_db), redis_client=Depends(get_redis)) -> Dict[str, Any]:
