...
async def chat_with_ai(
    ...
    try:
        ...
        response = await ai_service.generate_comprehensive_response(
            query=request.message, context=context, attachments=[]
        )
...
async def chat_with_ai_stream(request: ChatRequest, current_user: Dict = Depends(get_current_user)):
    """
    ...
    async def generate():
        try:
            ...
            final_response = await ai_service.generate_comprehensive_response(
                query=request.message, context={"user_id": current_user.get("id")}, attachments=[]
            )
...
async def analyze_portfolio(
    request: PortfolioAnalysisRequest, current_user: Dict = Depends(get_current_user)
):
    """
...
async def run_backtest(request: BacktestRequest, current_user: Dict = Depends(get_current_user)):
    ...
    try:
        results = await ai_service.run_backtest(
            strategy=request.strategy, symbol=request.symbol, period=request.period
        )
        return results
...
async def generate_chart(
    request: ChartGenerationRequest, current_user: Dict = Depends(get_current_user)
):
    """
    ...
    try:
        chart_base64 = await ai_service.generate_trading_chart(
            symbol=request.symbol, period=request.period, indicators=request.indicators
        )
...
async def process_multimodal_request(
    ...
    try:
        ...
        response = await ai_service.generate_comprehensive_response(
            query=message,
            context={"user_id": current_user.get("id"), "voice_enabled": voice_enabled},
...
async def log_interaction(session_id: str, user_id: str, message: str, response: str):
    """
