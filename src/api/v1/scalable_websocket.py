...
async def websocket_endpoint(
    websocket: WebSocket, token: Optional[str] = Query(None), client_id: Optional[str] = Query(None)
):
    """
...
async def market_data_websocket(
    ...
    try:
        # Connect
        connection_id = await manager.connect(
            websocket=websocket, metadata={"type": "market_data", "symbol": symbol}
        )
...
async def signals_websocket(
    ...
    try:
        ...
        # Connect
        connection_id = await manager.connect(
            websocket=websocket, metadata={"type": "signals", "symbols": symbol_list}
        )
...
async def get_websocket_stats(manager: ScalableWebSocketManager = Depends(get_ws_manager)):
    ...
    return JSONResponse(
        content={"status": "healthy", "stats": stats, "timestamp": datetime.utcnow().isoformat()}
    )
...
async def broadcast_message(
    ...
    if target_type in ["symbol", "user"] and not target_id:
        raise HTTPException(
            status_code=400, detail=f"target_id required for {target_type} broadcast"
        )
    ...
    return {"success": True, "recipients": count, "timestamp": datetime.utcnow().isoformat()}


@router.post("/market-data/{symbol}")
async def send_market_data(
    symbol: str, data: Dict[str, Any], manager: ScalableWebSocketManager = Depends(get_ws_manager)
):
    """Send market data update to symbol subscribers"""
    await manager.send_market_data(symbol.upper(), data)

    return {"success": True, "symbol": symbol.upper(), "timestamp": datetime.utcnow().isoformat()}
...
async def send_user_alert(
    ...
    return {"success": True, "user_id": user_id, "timestamp": datetime.utcnow().isoformat()}
...
def setup_scalable_websocket(app: FastAPI, redis_url: str):
    ...
    async def startup_websocket():
        ...
        ws_manager = ScalableWebSocketManager(redis_url=redis_url, server_id=server_id)
...
class MarketDataService:
    ...
    async def publish_price_update(self, symbol: str, price: float, volume: int):
        ...
        await self.ws_manager.send_market_data(
            symbol,
            {
                "price": price,
                "volume": volume,
...
class SignalService:
    ...
    async def publish_signal(
        self, symbol: str, signal_type: str, confidence: float, metadata: Dict[str, Any]
    ):
