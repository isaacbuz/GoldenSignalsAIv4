...
from src.services.market_data_service_mock import MockMarketDataService as MarketDataService
...
async def websocket_endpoint(websocket: WebSocket, token: Optional[str] = Query(None)):
    ...
    connection = ConnectionInfo(
        websocket=websocket, connection_id=connection_id, user_id=user_id, connection_type="general"
    )
    ...
    if channel == "market_data":
        ...
        if symbol:
            if subscribe:
                ...
                if not any(
                    f"market_data:{symbol}" in conn.subscriptions
...
async def handle_request(connection: ConnectionInfo, data: Dict[str, Any]):
    ...
    if action == "market_data":
        ...
        await connection.websocket.send_json(
            {"type": "agent_status", "data": metrics, "timestamp": datetime.utcnow().isoformat()}
        )
...
async def stream_signals(connection: ConnectionInfo):
    ...
    try:
        while True:
            # Get symbols this connection is subscribed to
            subscribed_symbols = {
                sub.split(":")[1] for sub in connection.subscriptions if sub.startswith("signals:")
            }
...
async def stream_agent_status(connection: ConnectionInfo):
    ...
    try:
        while True:
            ...
            has_agent_subs = any(
                sub.startswith("agent_status:") for sub in connection.subscriptions
            )
            ...
            if has_agent_subs:
                ...
                await connection.websocket.send_json(
                    {
                        "type": "agent_status",
                        "data": {"summary": metrics["summary"], "timestamp": metrics["timestamp"]},
                        "timestamp": datetime.utcnow().isoformat(),
                    }
                )
                        ...
                        await connection.websocket.send_json(
                            {
                                ...
                                "data": {
                                    ...
                                    "status": "active"
                                    if agent_metrics["total_signals"] > 0
                                    else "idle",
                                },
                                "timestamp": datetime.utcnow().isoformat(),
                            }
...
async def market_data_websocket(websocket: WebSocket, symbol: str):
    ...
    connection_id = str(uuid4())
    connection = ConnectionInfo(
        websocket=websocket, connection_id=connection_id, connection_type="market_data"
    )
...
async def signals_websocket(websocket: WebSocket):
    ...
    connection_id = str(uuid4())
    connection = ConnectionInfo(
        websocket=websocket, connection_id=connection_id, connection_type="signals"
    )
